...
const aggregateToCandlesticks = (prices: PriceHistoryDoc[], interval: number, priceType: 'price0' | 'price1'): CandlestickData[] => {
  if (!prices || prices.length === 0) return [];
  
  const candles = new Map<number, { open: number; high: number; low: number; close: number; firstTimestamp: number }>();
 ...
 */
export const useHistoricalPrices = (networkId: string, poolId: string, interval: number, priceType: 'price0' | 'price1') => {
  ...
  useEffect(() => {
    const fetchAndProcessData = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:4000/api/prices?${params.toString()}`);
        console.log(response);
        if (!response.ok) throw new Error(`Failed to fetch data: ${response.statusText}`);
        
        const rawData: PriceHistoryDoc[] = await response.json();
        
        // This hook no longer needs to calculate the interval; it just uses the one provided.
        const candleData = aggregateToCandlesticks(rawData, interval, priceType);
        setData(candleData);
      } catch (err: any) {
